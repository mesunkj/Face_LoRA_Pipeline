...
9. **防呆機制導致的靜默略過 (Silent Skip/Failure in Execution Loop)**
   - **原因**：在執行評估階段時，若發生「找不到靶圖」、「Ground Truth 無法偵測到人臉」或「產出圖片無人臉」等異常狀況，舊版程式採用了 `continue` 或 `return 0.0` 進行靜默防呆。這會導致日誌表面上看似成功執行完畢，但 `evaluation_matrix.csv` 的紀錄筆數與產出圖檔卻異常短缺，造成排障像無頭蒼蠅一樣困難。
   - **解法**：已全面導入「顯式警告 (Explicit Logging)」。現在任何觸發跳過的情境，系統皆會在終端機強制印出包含 **圖檔代號 (Identifier)** 與 **真實檔名 (Filename)** 的 `⚠️ 警告` 訊息。若後續發現 CSV 紀錄短缺，使用者可直接從日誌中透過代號快速定位出問題點。
